...
var pointsCounter = 0,
  points3dayCounter = 0,
  points8weekCounter = 0,
  pointsLast3dayCounter = 0,
  postsCounter = 0,
  posts3dayCounter = 0;
...
function setup() {
  ...
  var p5Canvas = createCanvas(
    width,
    width,
    WEBGL,
    document.getElementById("haloCanvas")
  );
...
function draw() {
  ...
  for (var i = 0; i < 1.0; i += 1.0 / ringCount) {
    ...
    stroke(
      mainColor.red * (i - colorRatio) +
        centerColor.red * (1.0 - i + colorRatio),
      mainColor.green * (i - colorRatio) +
        centerColor.green * (1.0 - i + colorRatio),
      mainColor.blue * (i - colorRatio) +
        centerColor.blue * (1.0 - i + colorRatio),
      255
    );
    ...
    /*if (i < 1.0/2.0) {
        let ip = i*2.0;
        stroke(
            color1.red*(1.0-ip) + color2.red*ip,
            color1.green*(1.0-ip) + color2.green*ip,
            color1.blue*(1.0-ip) + color2.blue*ip, 255)
      } else if (i < 2.0/2.0) {
        let ip = (i-1.0/2.0)*2.0;
        stroke(
            color2.red*(1.0-ip) + color3.red*ip,
            color2.green*(1.0-ip) + color3.green*ip,
            color2.blue*(1.0-ip) + color3.blue*ip, 255)
      }*/

    //Wave ring case
    if (
      waveCount > 0 &&
      floor((i - waveOffset) * ringCount) % floor(ringCount / waveCount) == 0
    ) {
      strokeWeight(2);
      stroke(
    ...
    for (var t = 0; t < 1.0; t += 0.005) {
      ...
      var rNoise =
        ...
        noise(
          extent +
            extent * cos(t * PI * 2 + i * PI * twist) +
            linearVelocity * cos(warpAngle) * time * speed,
          extent +
            extent * sin(t * PI * 2 + i * PI * twist) +
            linearVelocity * sin(warpAngle) * time * speed,
