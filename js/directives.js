...
class DirectiveManager {
  ...
  static toLocalISOString(d) {
    return new Date(
      d.getTime() - d.getTimezoneOffset() * 60000 - 0 * 24 * 60 * 60 * 1000
    ).toISOString();
  }
  ...
  static loadDirectives() {
    ...
    if (directives.length === 0) {
      this.directivesList.innerHTML =
        '<div class="text-gray-400 text-center p-4">NO ACTIVE DIRECTIVES FOR TODAY<br>CHECK ADMIN PANEL FOR SCHEDULING</div>';
      return;
    }
    ...
    directives.forEach((directive) => {
      ...
      directiveEl.className =
        "directive-card flex items-start p-4 rounded-lg transition-all duration-300 relative overflow-hidden"; // FORMAT SCHEDULE INFO
      const scheduleInfo = this.formatScheduleInfo(directive);
      directiveEl.setAttribute('data-tier', directive.tier);
      directiveEl.setAttribute('data-category', directive.category);
      if (isCompleted) {
        directiveEl.classList.add('completed');
      }
      ...
      >
      <div class="flex-1">
      <div class="directive-text ${isCompleted ? "line-through opacity-60" : ""}">${
        directive.text
      }</div>
      <div class="directive-meta">
        <span class="directive-tier">${directive.tier}</span> • 
        <span class="directive-category">${directive.category}</span>
      </div>
      <div class="directive-meta directive-schedule">${scheduleInfo.dates} • ${
        scheduleInfo.pattern
      }</div>
      </div>
      <button 
      class="directive-edit-btn"
    ...
    // Update collapsible container height if expanded
    this.recalculateDirectivesHeight();
  }

  static recalculateDirectivesHeight() {
  ...
  static editDirective(directiveId) {
    ...
    if (!directive) return; // SHOW ADMIN PANEL

    document.getElementById("adminModal").classList.add("active"); // POPULATE FORM WITH DIRECTIVE DATA
    document.getElementById("directiveText").value = directive.text;
    document.getElementById("directiveTier").value = directive.tier || "TIER 1";
    ...
    document.getElementById("recurrencePattern").disabled =
      !directive.recurring;
    document.getElementById("recurrencePattern").value =
      directive.recurrencePattern || "daily"; // SWITCH TO EDIT MODE

    document.getElementById("adminTitle").textContent =
      "ADMIN - EDIT DIRECTIVE";
    document.getElementById("addDirective").textContent = "UPDATE DIRECTIVE";
    document
      .getElementById("addDirective")
      .setAttribute("data-edit-id", directiveId); // SHOW CANCEL AND DELETE BUTTON
  ...
  static cancelEdit() {
    ...
    document.getElementById("directiveTier").value = "TIER 1";
    document.getElementById("directiveCategory").value = "DIAGNOSTICS";
    document.getElementById("startDate").value = "";
    document.getElementById("endDate").value = "";
    document.getElementById("recurringToggle").checked = false;
    document.getElementById("recurrencePattern").disabled = true; // SWITCH BACK TO ADD MODE
    ...
    document.getElementById("addDirective").removeAttribute("data-edit-id"); // HIDE CANCEL BUTTON
  ...
  static isDirectiveActiveToday(directive, today) {
    ...
    const states = this.getDirectiveStates(); // CHECK IF TODAY IS BEFORE START DATE

    //if (states[`${directive.id}`]) return false;
    if (today < startDate) return false; // CHECK IF TODAY IS AFTER END DATE (IF SET)
    if (endDate && today > endDate) return false; // IF NOT RECURRING, ACTIVE ON START DATE ONLY
  ...
  static toggleDirective(directiveId, completed) {
    ...
    // Update visual state immediately before full reload
    const directiveCard = document.querySelector(`[data-directive-id="${directiveId}"]`).closest('.directive-card');
    if (directiveCard) {
      if (completed) {
        directiveCard.classList.add('completed');
      } else {
        directiveCard.classList.remove('completed');
      }
    }

    this.loadDirectives(); // TRIGGER SCORE UPDATE
    document.dispatchEvent(new CustomEvent("dataChange"));
    
    // Recalculate height after directive state change
    this.recalculateDirectivesHeight();
  }
  ...
  static calculateCompletionStreak() {
    ...
    for (let dayOffset = 0; dayOffset < 365; dayOffset++) {
      ...
      const completionRate =
        activeOnDate.length > 0 ? completedOnDate / activeOnDate.length : 0;
  ...
  static updateDirective(
    ...
    if (directiveIndex === -1) return; // UPDATE DIRECTIVE
  ...
  static removeDirective(directiveId) {
    const directives = this.getDirectives();
    const today = this.toLocalISOString(new Date()).split("T")[0]; // YYYY-MM-DD FORMAT
    console.log(directives); // CLEAN UP STATES
